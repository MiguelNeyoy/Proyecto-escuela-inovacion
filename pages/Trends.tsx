...
const Trends: React.FC = () => {
  ...
  return (
    <PageContainer className="flex min-h-screen flex-col bg-background-light dark:bg-background-dark font-body">
      <TopBar title="Tendencias" />
      
      {/* Tabs */}
      <div className="sticky top-[56px] z-10 bg-background-light/90 dark:bg-background-dark/90 backdrop-blur-sm px-4 border-b border-gray-200 dark:border-gray-800 animate-slide-in-down">
        <div className="flex overflow-x-auto no-scrollbar gap-6">
          {['Todos', 'Procesadores', 'Tarjetas de Video', 'Placas Base', 'RAM'].map((tab, i) => (
             <button 
                ...
                style={{ animation: `slideInLeft 0.4s ease-out ${i * 0.08}s backwards` }}
             >
               {tab}
             </button>
          ))}
...
const TrendCard: React.FC<{ title: string, subtitle: string, icon: string, img: string, visible: boolean, index: number }> = ({ title, subtitle, icon, img, visible, index }) => (
  <div className="bg-white dark:bg-card-dark p-3 rounded-xl shadow-sm flex flex-col gap-3" style={{
    animation: visible ? `zoomIn 0.5s ease-out ${index * 0.12}s backwards` : 'none'
  }}>

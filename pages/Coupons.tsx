...
const Coupons: React.FC = () => {
  return (
    <div className="flex flex-col min-h-screen bg-background-light dark:bg-background-dark font-body">
      <TopBar title="Cupones" />
      
      {/* Chips */}
      <div className="flex gap-3 px-4 py-2 overflow-x-auto no-scrollbar">
        <Chip label="Todos" active />
        <Chip label="Tarjetas Gráficas" />
        <Chip label="Procesadores" />
        <Chip label="Monitores" />
      </div>
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        />
        <div className="flex flex-col items-center justify-center gap-4 rounded-xl bg-white dark:bg-card-dark p-8 mt-4 text-center">
...
const Chip: React.FC<{ label: string, active?: boolean }> = ({ label, active }) => (
  <button className={`flex h-8 shrink-0 items-center justify-center rounded-lg px-4 text-sm font-medium transition-colors ${active ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-800 text-text-light dark:text-white'}`}>
    {label}
  </button>
);

const CouponCard: React.FC<{ discount: string, desc: string, code: string, expiry: string, logo: string }> = ({ discount, desc, code, expiry, logo }) => (
  <div className="flex flex-col gap-3 rounded-xl bg-white dark:bg-card-dark p-4 shadow-sm border border-gray-100 dark:border-gray-800">

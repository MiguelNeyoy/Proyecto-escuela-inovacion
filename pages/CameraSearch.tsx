...
const CameraSearch: React.FC = () => {
  ...
  return (
    <PageContainer className="flex h-screen flex-col bg-black">
      {/* Header Overlay */}
      <header className="absolute top-0 w-full z-10 flex items-center justify-between p-6 text-white animate-slide-in-down">
        <button onClick={() => navigate(-1)} className="p-2 -ml-2 rounded-full bg-black/20 backdrop-blur-md hover:bg-black/40 transition-colors">
      ...
      <main className="relative flex-grow flex items-center justify-center overflow-hidden">
        ...
        {/* Overlay UI */}
        <div className="absolute top-24 text-center w-full px-6 animate-slide-in-down" style={{ animationDelay: '0.1s' }}>
        ...
        {/* Scan Frame */}
        <div className="relative w-64 h-64 border-2 border-white/50 rounded-3xl animate-zoom-in" style={{ animationDelay: '0.2s' }}>
      ...
      {/* Controls */}
      <footer className="bg-black text-white p-8 pb-10 animate-slide-in-up">
        <div className="flex justify-around items-center">
          <button className="flex flex-col items-center space-y-1 opacity-80 hover:opacity-100 transition-opacity hover:scale-110 transform" style={{ animation: `zoomIn 0.4s ease-out 0.3s backwards` }}>
          ...
          <button onClick={() => navigate('/results')} className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(255,255,255,0.3)] active:scale-95 transition-transform hover:shadow-[0_0_30px_rgba(255,255,255,0.5)]" style={{ animation: `zoomIn 0.4s ease-out 0.35s backwards` }}>
            <div className="w-16 h-16 bg-transparent border-4 border-primary rounded-full"></div>
          </button>
          
          <button className="flex flex-col items-center space-y-1 opacity-80 hover:opacity-100 transition-opacity hover:scale-110 transform" style={{ animation: `zoomIn 0.4s ease-out 0.4s backwards` }}>
             <div className="p-3 bg-white/10 rounded-full">
